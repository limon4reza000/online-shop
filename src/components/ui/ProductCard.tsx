...
export function ProductCard({
  ...
  return (
    <div
      className={`group relative flex flex-col h-full bg-white hover:border-primary hover:shadow-lift transition-[border-color,box-shadow] duration-300 ease-out ${
        bordered ? 'border-r border-b border-border' : 'border border-border overflow-hidden'
      }`}
    >
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          ...
          {product.tags.includes('new') && (
            <span className="rounded-full bg-text-primary text-white text-[11px] font-bold px-2.5 py-1">{t('common.new')}</span>
          )}
        </div>
