...
const addressFields: { key: keyof typeof initialDetails; label: string }[] = [
...
export default function Cart() {
  const { items, updateQuantity, removeItem, subtotal, deliveryCharge, vat, coupon, discount, applyCoupon, removeCoupon } = useCart();
  const [couponInput, setCouponInput] = useState('');
  const [details, setDetails] = useState(initialDetails);
  const navigate = useNavigate();

  const updateDetail = (key: keyof typeof initialDetails, value: string) => setDetails((prev) => ({ ...prev, [key]: value }));
                ...
                <div key={`${item.productId}-${item.color}-${item.size}`} className="flex gap-4 card-surface p-4">
                  ...
                  <div className="flex flex-col items-end justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-2 bg-gray-100 rounded-full p-1">
                      ...
                      <button onClick={() => updateQuantity(item.productId, item.quantity + 1, item.color, item.size)} className="grid place-items-center h-7 w-7 rounded-full bg-primary text-white hover:bg-primary-hover transition-colors">
                        <Plus size={12} />
                      </button>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={(e) => updateDetail('mobile', e.target.value)}
              ...
              <div className="card-surface p-6">
                ...
                {coupon && (
                  <div className="mt-2 flex items-center justify-between text-xs bg-success/10 text-success rounded-lg px-3 py-2">
                    ...
                    <button onClick={removeCoupon} className="font-semibold hover:underline">সরান</button>
                  </div>
                )}
